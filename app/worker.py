...
class Worker:
    ...
    async def download_repository(self, task: dict) -> tuple[str, str, str]:
        ...
        temp_dir = None
        try:
            # self.redis_client.update_task_status(task["task_id"], "downloading") -> Задача уже становится downloading при назначении
            ...
            self.logger.info(f"Репозиторий скачан успешно в '{extracted_path}'")
            return extracted_path, temp_dir, "Success"
            
        except Exception as e:
            self.logger.error(f"Ошибка скачивания репозитория: {e}")
    ...
    async def extract_zip_file(self, task: dict) -> tuple[str, str, str]:
        ...
        try:
            ...
            self.logger.info(f"Распаковываю ZIP файл для '{project_name}': '{zip_file_path}'")
            ...
            try:
                with zipfile.ZipFile(zip_file_path, 'r') as zip_file:
                    ...
                    self.logger.info(f"Начинаю извлечение {total_files} файлов из ZIP")
                    ...
                    for i, file_info in enumerate(files_list):
                        ...
                        try:
                            ...
                            self.logger.warning(f"Ошибка извлечения файла '{file_info.filename}': {e}")
                            continue
                    
                    # Финальный прогресс
                    self.send_heartbeat("unpacking", force=True, progress=100, progress_detail=f"Извлечено {total_files} файлов")
                    
                    self.logger.info(f"Распаковка завершена: {total_files} файлов")
            ...
            self.logger.info(f"ZIP файл распакован успешно ('{extracted_size // 1024}KB')")
    ...
    async def scan_repository(self, task: dict, repo_path: str) -> tuple[list, dict]:
        ...
        try:
            ...
            project_name = task["project_name"]
            self.logger.info(f"Начинаю сканирование '{project_name}'")
            start_time = time.time()
            ...
            self.logger.info(f"Применяю ML фильтрацию для '{project_name}'")
            try:
            ...
            self.logger.info(f"Обработка '{project_name}' завершена за {total_time:.2f}с. Финальных секретов: '{len(filtered_results)}'")
    ...
    async def send_callback(self, task: dict, payload: dict):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(f"Отправляю callback для '{project_name}'. Результатов: '{results_count}'. Сжатие: '{compression_ratio:.1f}'% (попытка {attempt + 1})")
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.post(callback_url, data=compressed_json, headers=headers) as response:
                        if response.status == 200:
                            self.logger.info(f"Callback успешно отправлен для '{project_name}'")
                            return
                        else:
                            self.logger.error(f"Callback failed: HTTP '{response.status}'")
                            response_text = await response.text()
                            self.logger.error(f"Response: {response_text[:500]}")
                            
                            if attempt < max_retries - 1:
                                self.logger.info(f"Retry callback через '{retry_delay}' секунд...")
                                await asyncio.sleep(retry_delay)
                                retry_delay *= 2
                            else:
                                raise Exception(f"Callback failed after '{max_retries}' attempts: HTTP '{response.status}'")
                
            except asyncio.TimeoutError:
                self.logger.error(f"Callback timeout на попытке '{attempt + 1}'")
                if attempt < max_retries - 1:
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2
                else:
                    raise Exception(f"Callback timeout after '{max_retries}' attempts")
            except Exception as e:
                self.logger.error(f"Ошибка отправки callback (попытка '{attempt + 1}'): {e}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2
                else:
    ...
    async def process_task(self, task: dict):
        ...
        try:
            ...
            self.logger.error(f"Ошибка обработки задачи: {error_msg} (время: {execution_time:.2f}с)")
            ...
            try:
                if self.current_task and "original_data" in self.current_task:
                    ...
                    try:
                        ...
                        self.logger.warning("Не удалось отправить error callback")
                        
            except Exception as callback_error:
                self.logger.error(f"Ошибка отправки error callback: {callback_error}")
            ...
            try:
                ...
                self.logger.warning(f"Ошибка cleanup: {cleanup_error}")
